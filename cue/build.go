...
type index struct {
	...
	importsByPathMutex sync.RWMutex
...
func (idx *index) findLabel(s string) (f label, ok bool) {
	for x := idx; x != nil; x = x.parent {
		idx.labelMapMutex.RLock()
		f, ok = x.labelMap[s]
		idx.labelMapMutex.RUnlock()
		if ok {
			break
		}
	}
...
func (idx *index) label(s string, isIdent bool) label {
	...
	if !ok {
		...
		idx.labelMapMutex.Lock()
		idx.labelMap[s] = f
		idx.labelMapMutex.Unlock()
